...
model.add(Dense(xMaxLen*yDimension, activation='softmax'))
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
checkpointer = ModelCheckpoint(filepath='model/entityDetection' + str(yDimension) + "." + modelType + '.model', verbose=1, save_best_only=True,)
history = LossHistory()
result = model.fit(X[:50000], Y[:50000], batch_size=batchSize, nb_epoch=epoch, verbose=1, validation_data=(X[50001:60000], Y[50001:60000]), callbacks=[checkpointer, history])
model.save('model/entityDetection' + str(yDimension) + "." + modelType + '.model')
score = model.evaluate(X[60001:80000], Y[60001:80000], batch_size=batchSize)
print (score)
